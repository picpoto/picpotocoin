...
*Setup instructions for a gitian build of PPO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PPO
...
Clone the git repositories for ppo and gitian and then checkout the ppo version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ppo repository
...
To build PPO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ppo repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ppo/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/PPOCRYPTO/PPO
...
./bin/gbuild --commit ppo=${COMMIT} --url ppo=${URL} ../ppo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ppo=${COMMIT} --url ppo=${URL} ../ppo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ppo=${COMMIT} --url ppo=${URL} ../ppo/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ppo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ppo-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ppo-build.assert
```
...
[ppo/gitian.sigs](https://github.com/ppo-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
