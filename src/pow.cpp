...
{
    /* current difficulty formula, ppo - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
	...
	if(pindexLast->nHeight >= 200 && pindexLast->nHeight <= LAST_POW_BLOCK(chainActive.Height())) {
        uint256 bnNew;
		bnNew = ~uint256(0) >> 20;
		return bnNew.GetCompact();
	}

	if(pindexLast->nHeight == 19270) {
        uint256 bnNew;
		bnNew = ~uint256(0) >> 20;
		return bnNew.GetCompact();
	}
